...
module.exports = {
  ...
  //"no-debugger": 0, //不允许出现debugger语句
    "no-dupe-args": 0, //函数定义的时候不允许出现重复的参数
    "no-dupe-keys": 0, //对象中不允许出现重复的键
    "no-duplicate-case": 0, //switch语句中不允许出现重复的case标签
    ...
    "no-invalid-regexp": 0, //不允许在RegExp构造函数里出现无效的正则表达式
    ...
    "no-param-reassign": 0, //不允许重新分配函数参数"no-proto": 0, //不允许使用__proto__属性
    "no-redeclare": 0, //不允许变量重复声明
    ...
    "no-with": 0, //不允许使用with语句
    "radix": 0, //使用parseInt时强制使用基数来指定是十进制还是其他进制
    "vars-on-top": 0, //var必须放在作用域顶部
    "wrap-iife": [0, "any"], //立即执行表达式的括号风格
    "yoda": [0, "never", {"exceptRange": true}], //不允许在if条件中使用yoda条件
    "strict": [0, "function"], //使用严格模式
    "no-catch-shadow": 0, //不允许try catch语句接受的err变量与外部变量重名"no-delete-var": 0, //不允许使用delete操作符
    ...
    "no-use-before-define": [0, "nofunc"], //不允许在未定义之前就使用变量"indent": 0, //强制一致的缩进风格
    "brace-style": [0, "0tbs", { "allowSingleLine": false}], //大括号风格
    ...
    "no-multiple-empty-lines": [0, {"max": 0}], //空行最多不能超过两行
    ...
    "space-infix-ops": [0, {"int30Hint": true}], //操作符周围的空格
