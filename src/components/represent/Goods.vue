      ...
      <ul id="go-list" v-for="(items, index) in productInfo" :key="index">
          <li>
            <div class="check">
              ...
              <input :value="index" type="radio" :id="index" name="choose" @change="checkDefault(index)">
              <label id="con-box" :for="index"></label>
            </div>
            <div id="con-goods">
              <div>
...
export default {
  ...
  methods: {
    ...mapActions(['getProductInfoByName','changeProduct']),
    finish(){
      if(this.product){
    ...
    checkDefault(i){
      ...
      this.product = this.productInfo[i];
    }
  },
  watch: {
    words(curVal, oldVal) {
